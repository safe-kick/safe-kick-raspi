...
Raspberry Pi 연결
  ...
  -> 주행 5초 후 첫 유효 샘플을 탑승자 기준 무게로 저장
  -> 기준보다 10kg 증가한 샘플 2회 연속: BUZZ_ON
  -> 경고가 추가 5초 지속: LOCK
```
...
- 기준값 대비 증가량이 `2000` 이상인 표본이 `3`회 연속 발생
- 원시 센서 표본 중 하나라도 `1500`을 초과

두 조건은 OR로 적용됩니다. 따라서 원시값 `1500`은 절대 임계값 조건을 통과하고,
`1501`부터 음주로 판정됩니다. 기준값이 없거나 표본이 8개보다 적으면 유효하지
않은 호흡으로 보고 재시도합니다. MQ-3가 정상 범위인 경우에는 HW-484 호흡을
0.5초 이상 확인해야 통과합니다. 다만 MQ-3가 음주 임계값을 넘으면 호흡 감지
여부와 관계없이 안전 우선으로 잠급니다. UART mock의 정상 표본은 실제 장비에서 관찰한 90 전후의
원시값 범위에 맞춰져 있습니다.
...
HW-484 단독 확인은 다음처럼 실행합니다. 기본 HIGH(1), 감지 LOW(0)이며 0.5초
...
## STM32 UART 테스트 도구 실행

> **안전 주의:** Uvicorn 서버와 `kickserial`은 같은 UART 포트를 동시에 사용할
> 수 없습니다. 이 절은 Raspberry Pi에서 테스트 도구를 실행하는 방법만 설명합니다.
> 센서·모터별 배선, 시험 절차, 정상 기준과 결과는 STM32 저장소에서 관리합니다.
...
로드셀·MQ-3·릴레이·부저·L298N·모터의 상세 배선, 시험 순서, 정상 판정 기준과
실제 시험 결과는
[safe-kick-stm32 시리얼 시험 문서](https://github.com/safe-kick/safe-kick-stm32/blob/main/README_SERIAL_TEST.md)를
참고합니다. STM32 하드웨어 시험 내용은 해당 문서를 단일 원본으로 관리합니다.
...
센서값, 배선, 보정 또는 모터 동작 문제는
[STM32 문제 해결 및 시험 기준](https://github.com/safe-kick/safe-kick-stm32/blob/main/README_SERIAL_TEST.md)을
확인합니다.
